...
def pso_2_bin(ind_list, bounds, vbounds):
  IndividualPSO.gi = None
  for i in range(0,len(ind_list)):
    ind_list[i] = ind_list[i].cloud_particle_loop(ind_list,bounds,vbounds)
  return ind_list[:]
...
def deepso_rand_1(ind_list, tmut, tcom, bounds, vBounds, wBounds):
  for i in range(0,len(ind_list)):
    ind_list[i] = ind_list[i].rand_1(ind_list, tmut, tcom, bounds=bounds, vBounds=vBounds, wBounds=wBounds)
  return ind_list[:]
