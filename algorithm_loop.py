...
def initial_pop_generator(pop_size,classname,map_size,center,*args):
  coordinates = []
  sqrt = math.sqrt
  floor = math.floor
...
def genetic_algorithm(function_name,recombination_name,mutation_name,sigma,classname,map_size,parent_size=50,population_size=100,center=[0,0],alpha=0.02,max_it=50):
  population = initial_pop_generator(population_size,classname,map_size,center,alpha)
  generation_data=[]
  gen_number=0
...
def particle_swarm_optimization(experiment_name,classname,map_size,bounds,population_size=100,center=[0,0],max_it=50,C1=0.8,C2=0.7,W=0.2):
  population = initial_pop_generator(population_size,classname,map_size,center,C1,C2,W)
  generation_data=[]
  gen_number=0
  ...
  while gen_number < max_it:
    ...
    fun = getattr(sys.modules[__name__],experiment_name)
...
def diferencial_evolution(experiment_name,classname,map_size,bounds,population_size=100,center=[0,0],R=0.8,Cr=0.7,max_it=50):
  ...
  population = initial_pop_generator(population_size,classname,map_size,center,R,Cr,min_b,diff)
  generation_data=[]
  gen_number=0
  ...
  while gen_number < max_it:
    ...
    fun = getattr(sys.modules["auxiliarfunctions"],experiment_name)
    population = fun(population)
